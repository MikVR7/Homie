...
# OLD BROKEN ENDPOINT REMOVED - NOW USING execute-operations ONLY!

@app.route('/api/file-organizer/execute-operations', methods=['POST'])
def execute_operations():
    """Execute abstract operations generated by AI - CROSS-PLATFORM ARCHITECTURE!"""
    try:
        data = request.get_json()
        operations = data.get('operations', [])
        explanations = data.get('explanations', [])
        fallback_operations = data.get('fallback_operations', [])
        ...
        print(f"[API] Execute operations request:")
        print(f"[API]   Operations: {len(operations)} total")
        print(f"[API]   Fallbacks: {len(fallback_operations)} available")
        for i, op in enumerate(operations):
            print(f"[API]   Operation {i+1}: {op}")
        ...
        # Execute the abstract operations
        result = organizer.execute_operations(operations, explanations, fallback_operations)
        
        print(f"[API] Operations executed: {result['successful']}/{result['total_operations']} successful on {result['platform']}")
        return jsonify({
        ...
        error_msg = f"Unexpected error executing operations: {str(e)}"
...
@app.route('/api/file-organizer/start-drive-monitoring', methods=['POST'])
def start_drive_monitoring():
    ...
    try:
        ...
        organizer.start_drive_monitoring()
        
        return jsonify({
            'success': True,
            'message': 'Drive monitoring started',
            'monitoring_active': True
        })
        
    except Exception as e:
        print(f"Error starting drive monitoring: {str(e)}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500

@app.route('/api/file-organizer/stop-drive-monitoring', methods=['POST'])
def stop_drive_monitoring():
    ...
    try:
        ...
        organizer.stop_drive_monitoring()
        
        return jsonify({
            'success': True,
            'message': 'Drive monitoring stopped',
            'monitoring_active': False
        })
        
    except Exception as e:
        print(f"Error stopping drive monitoring: {str(e)}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500

@app.route('/api/file-organizer/drive-status', methods=['GET'])
def get_real_time_drive_status():
    """Get real-time drive status and events"""
    try:
        from dotenv import load_dotenv
        load_dotenv()
        api_key = os.getenv('GEMINI_API_KEY')
        ...
        # Get real-time drive status
        result = organizer.get_real_time_drive_status()
        
        return jsonify(result)
        
    except Exception as e:
        print(f"Error getting drive status: {str(e)}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500

@app.route('/api/file-organizer/folder-analytics', methods=['GET'])
def get_folder_analytics():
    """Get detailed folder usage analytics and patterns"""
    try:
        from dotenv import load_dotenv
        load_dotenv()
        api_key = os.getenv('GEMINI_API_KEY')
        ...
        # Get destination memory and enhance with analytics
        destination_memory = organizer.get_destination_memory()
        
        # Calculate analytics
        analytics = {
            'total_mappings': len(destination_memory.get('category_mappings', {})),
            'category_stats': {},
            'usage_patterns': [],
            'confidence_scores': {},
            'recent_activity': destination_memory.get('recent_destinations', [])[:10]
        }
        
        # Process category statistics
        for category, destinations in destination_memory.get('category_mappings', {}).items():
            total_usage = sum(destinations.values())
            most_used_dest = max(destinations.keys(), key=destinations.get) if destinations else None
            
            analytics['category_stats'][category] = {
                'total_usage': total_usage,
                'unique_destinations': len(destinations),
                'most_used_destination': most_used_dest,
                'destinations': destinations
            }
        
        # Process confidence scores
        for category, paths in destination_memory.get('pattern_confidence', {}).items():
            analytics['confidence_scores'][category] = {
                path: data['confidence'] for path, data in paths.items()
            }
        ...
        print(f"Error getting folder analytics: {str(e)}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
...
if __name__ == '__main__':
    ...
    # OLD execute-action endpoint REMOVED - using execute-operations only!
    print("    POST /api/file-organizer/execute-operations")
    ...
    print("    POST /api/file-organizer/start-drive-monitoring")
    print("    POST /api/file-organizer/stop-drive-monitoring")
    print("    GET  /api/file-organizer/drive-status")
    print("    GET  /api/file-organizer/folder-analytics")
